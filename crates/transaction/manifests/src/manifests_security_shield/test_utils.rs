...
use radix_engine::{object_modules::role_assignment::*, system::system_db_reader::*};
...
where
    ...
    D: TestDatabase,
{ 
    fn read_access_controller_state(
        &mut self,
        address: AccessControllerAddress
    ) -> AccessControllerV2StateV2 {
        let payload: AccessControllerV2StateFieldPayload = self.component_state(address.scrypto().try_into().unwrap());

        let payload_content = payload.into_content();
        let versioned = payload_content.into_versions();
        match versioned {
            AccessControllerV2StateVersions::V1(old) => panic!("unsupported"), // From<V1> is implemented
    ...
    fn read_access_controller_rule_set(
        ...
        ScryptoRuleSet {
            primary_role: self.read_access_controller_access_rule(address.clone(), "primary"),
            recovery_role: self.read_access_controller_access_rule(address.clone(), "recovery"),
            confirmation_role: self.read_access_controller_access_rule(address.clone(), "confirmation"),
        }
    }

    // Create an Access Controller for the given account with the given security structure of factor instances
    fn create_access_controller(
        ...
        let account_address = unsecurified_account.address().into_account().unwrap();
        // Preload account with free XRDz to use for fee payment.
        self.load_account_from_faucet(account_address.into());

        let mut securify_account_manifest = TransactionManifest::apply_security_shield_for_unsecurified_entity(
        ...
        securify_account_manifest = securify_account_manifest.modify_add_lock_fee(
        ...
        let notarized_tx = self.sign_and_notarize_transaction(securify_account_manifest, |intent| {
            let signature = unsecurified_account.unsecured_entity_control.transaction_signing.factor_instance().sign_hash(&intent.transaction_intent_hash().hash);
            SignedIntent::with_signatures(intent,vec![signature.into()]).unwrap()
        });
        
        let scrypto_ac_address = self.execute_notarized_transaction(notarized_tx)
    ...
    {
        let notary_private_key: Secp256k1PrivateKey = Secp256k1PrivateKey::sample();
        let notary_public_key: Secp256k1PublicKey = notary_private_key.public_key();

        let current_epoch = self.get_current_epoch();
        let tx_header = TransactionHeader::new(NetworkID::Simulator, current_epoch, current_epoch.after(10).unwrap(), 10, notary_public_key, true, 0);
        let intent = TransactionIntent::new(tx_header, manifest, Message::None).unwrap();
        let signed_intent = sign_intent(intent);

        let notary_signature = notary_private_key.notarize_hash(&signed_intent.hash());
        let notarized_transaction = NotarizedTransaction::new(signed_intent, notary_signature).unwrap();
...
impl SecuredEntityControl {
    fn sign_intent_with_roles(&self, intent: TransactionIntent, exercised_roles: RolesExercisableInTransactionManifestCombination) -> SignedIntent {
        ...
        let intent_signatures: Vec<IntentSignature> = signatures.into_iter().map(Into::into).collect();

        SignedIntent::with_signatures(intent, intent_signatures).expect("Should succeed building the signed intent")
    }
}