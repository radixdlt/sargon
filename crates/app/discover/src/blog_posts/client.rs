...
impl BlogPostsClient {
    ...
    pub async fn get_blog_posts(&self) -> Result<BlogPosts> {
        ...
        let fresh = self.refresh_from_remote().await?;
        Ok(BlogPosts::new(fresh, None))
    }

    async fn refresh_from_remote(&self) -> Result<Vec<BlogPost>> {
...
mod tests {
    ...
    use crate::blog_posts::client_remote_fetch::{
        FieldData, RemoteBlogPost, RemoteBlogPosts,
    };
    ...
    async fn success_empty_cache_remote_success() {
        ...
        let network_driver = MockNetworkingDriver::new_with_bodies(
            200,
            vec![blog_posts.serialize_to_bytes().unwrap().into()],
        );
        ...
        let expected_cached_blog_posts =
            CachedBlogPosts::new(expected_blog_posts.posts);
        pretty_assertions::assert_eq!(
            cached_blog_posts,
    ...
    async fn existing_cache_no_remote_updates() {
        ...
        let cached_blog_posts =
            CachedBlogPosts::new(BlogPosts::sample().posts.clone());

        let network_driver = MockNetworkingDriver::new_with_bodies(304, vec![]);
        file_system_client
            .save_to_file(
    ...
    async fn existing_cache_remote_updates_exist_failed_to_fetch_new_blog_posts(
        ...
        let network_driver = MockNetworkingDriver::new_with_bodies(404, vec![]);
        file_system_client
            .save_to_file(
                CACHE_BLOG_POSTS_PATH,
                cached_blog_posts.serialize_to_bytes().unwrap(),
    ...
    async fn existing_cache_remote_updates_exist_refresh_with_new_blog_posts() {
        ...
        let cached_blog_posts =
            CachedBlogPosts::new(vec![old_blog_post.clone().into()]);
        ...
        let network_driver = MockNetworkingDriver::new_with_bodies(
            200,
            vec![new_remote_blog_posts.serialize_to_bytes().unwrap().into()],
        );
        file_system_client
            .save_to_file(
                CACHE_BLOG_POSTS_PATH,
