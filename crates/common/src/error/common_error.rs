...
pub enum CommonError {
    ...
    AccountOnWrongNetwork { expected: u8, found: u8 } = 10053,
    ...
    ProfileSnapshotNotFound { bad_value: String } = 10075,

    #[error("Account Already Present {bad_value}")]
    AccountAlreadyPresent { bad_value: String } = 10076,
    ...
    UnableToSaveMnemonicToSecureStorage { bad_value: String } = 10078,

    #[error(
        "Failed load Mnemonic from SecureStorageDriver with FactorSourceID: {bad_value}"
    )]
    UnableToLoadMnemonicFromSecureStorage { bad_value: String } = 10079,

    #[error("Failed save FactorSource to SecureStorageDriver, FactorSourceID: {bad_value}")]
    UnableToSaveFactorSourceToProfile { bad_value: String } = 10080,
    ...
    InvalidInstructionsWrongNetwork {
        found_in_instructions: u8,
        specified_to_instructions_ctor: u8,
    } = 10095,
    ...
    NetworkDiscrepancy { expected: u8, actual: u8 } = 10131,
    ...
    UnableToLoadProfileFromSecureStorage { profile_id: String } = 10138,
