...
impl CompiledTransactionIntent {
    ...
    pub fn new(bytes: BagOfBytes) -> Result<Self> {
        RET_intent_from_payload_bytes(bytes.clone())
            .map(|_| Self(bytes))
            .map_err(|_| {
...
impl TransactionIntent {
    pub fn compile(&self) -> CompiledTransactionIntent {
        let bytes =
            RET_intent_to_payload_bytes(&ScryptoIntent::from(self.clone()))
                .expect("Should always be able to compile an Intent");

        CompiledTransactionIntent(bytes.into())
    }
}
...
impl From<TransactionIntent> for CompiledTransactionIntent {
