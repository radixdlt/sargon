...
impl ArculusWalletClient {
    ...
    for path in paths {
        ...
        let result = self.get_public_key_by_path_io(wallet, path.clone(), CardCurve::Ed25519Curve).await?;
    ...
    async fn _create_wallet_seed(&self, wallet: ArculusWalletPointer, pin: String, word_count: i64) -> Result<Mnemonic> {
        let pin_len = pin.len() as i64;
