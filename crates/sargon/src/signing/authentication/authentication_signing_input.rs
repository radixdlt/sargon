    ...
    pub fn try_from_profile(
        ...
        let security_state = match address_of_entity {
            AddressOfAccountOrPersona::Account(account_address) => {
                profile.account_by_address(account_address).map(|a| a.security_state)
            }
            AddressOfAccountOrPersona::Identity(identity_address) => {
                profile.persona_by_address(identity_address).map(|a| a.security_state)
            }
        }?;
        ...
        let owned_factor_instance = OwnedFactorInstance::new(
            address_of_entity,
            factor_instance
        );
...
mod test {
    ...
    fn test_with_usecurified_account() {
        let expected_factor_instance = HierarchicalDeterministicFactorInstance::sample_fia0();
        ...
        assert_eq!(
            sut.owned_factor_instance.value,
            expected_factor_instance
        );

        assert_eq!(
            "6fc75ec1d5c00941dc587c0a07409da1740c423c337c323ba7bdf68d61d4dd8e".parse::<Hash>().unwrap(),
            sut.challenge.hash()
        )
    }
    ...
    fn test_with_usecurified_persona() {
        let expected_factor_instance = HierarchicalDeterministicFactorInstance::sample_fii0();
        ...
        let factor_sources = FactorSource::sample_all();
        let profile = Profile::sample_from(factor_sources, [], [&unsecurified_persona]);
