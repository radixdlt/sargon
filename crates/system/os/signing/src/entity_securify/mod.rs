...
pub async fn sign_entity_securify(
    ...
    let fee_payer_account = profile.account_by_address(fee_paying_account_address)?;
    ...
    let securified_entity_applying_shield = profile.entity_by_address(entity_address)?;
    let unsecurified_entity = AnyUnsecurifiedEntity::new(securified_entity_applying_shield)?;
    ...
    let securify_manifest = TransactionManifest::apply_security_shield_for_unsecurified_entity(
        unsecurified_entity.clone(), 
        unsecurified_entity.provisional_securified_config.clone().unwrap().get_security_structure_of_factor_instances(),
    )?;
