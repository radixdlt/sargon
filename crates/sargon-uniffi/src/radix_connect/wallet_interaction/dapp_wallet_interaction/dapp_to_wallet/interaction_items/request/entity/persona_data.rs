...
impl From<InternalDappToWalletInteractionPersonaDataRequestItem>
    for DappToWalletInteractionPersonaDataRequestItem
{
    fn from(
        value: InternalDappToWalletInteractionPersonaDataRequestItem,
    ) -> Self {
        Self {
            is_requesting_name: value.is_requesting_name,
            number_of_requested_email_addresses: value
                .number_of_requested_email_addresses
                .map(Into::into),
            number_of_requested_phone_numbers: value
...
impl Into<InternalDappToWalletInteractionPersonaDataRequestItem>
    for DappToWalletInteractionPersonaDataRequestItem
{
    fn into(self) -> InternalDappToWalletInteractionPersonaDataRequestItem {
        InternalDappToWalletInteractionPersonaDataRequestItem {
            ...
            number_of_requested_email_addresses: self
                .number_of_requested_email_addresses
                .map(Into::into),
            number_of_requested_phone_numbers: self
                .number_of_requested_phone_numbers
                .map(Into::into),
        }
    }
