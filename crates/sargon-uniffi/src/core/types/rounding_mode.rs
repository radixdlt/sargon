...
#[derive(Clone, PartialEq, Eq, strum::Display, uniffi::Enum)]
...
impl From<InternalRoundingMode> for RoundingMode {
    fn from(value: InternalRoundingMode) -> Self {
        match value {
            InternalRoundingMode::ToPositiveInfinity => {
                Self::ToPositiveInfinity
            }
            InternalRoundingMode::ToNegativeInfinity => {
                Self::ToNegativeInfinity
            }
            InternalRoundingMode::ToZero => Self::ToZero,
            InternalRoundingMode::AwayFromZero => Self::AwayFromZero,
            InternalRoundingMode::ToNearestMidpointTowardZero => {
                Self::ToNearestMidpointTowardZero
            }
            InternalRoundingMode::ToNearestMidpointAwayFromZero => {
                Self::ToNearestMidpointAwayFromZero
            }
            InternalRoundingMode::ToNearestMidpointToEven => {
                Self::ToNearestMidpointToEven
            }
        }
    }
}
