...
impl<InternalKey, InternalElement, Key, Element> MapFromInternalHashMap<InternalKey, InternalElement, Key, Element> for HashMap<InternalKey, InternalElement>
where
    Key: From<InternalKey> + std::hash::Hash + Eq,
    Element: From<InternalElement>,
{
    fn into_hash_map(self) -> HashMap<Key, Element> {
        self.into_iter().map(|(k, v)| (Key::from(k), Element::from(v))).collect()
    }
}

impl<InternalKey1, InternalElement, InternalKey2, Key1, Key2, Element> MapFromInternalHashMap<InternalKey1, InternalElement, Key1, HashMap<Key2, Element>> for HashMap<InternalKey1, HashMap<InternalKey2, InternalElement>>
where
    ...
    Element: From<InternalElement>,
{
    fn into_hash_map(self) -> HashMap<Key1, HashMap<Key2, Element>> {
        self.into_iter().map(|(k, v)| (Key1::from(k), v.into_hash_map())).collect()
    }
}

impl<InternalKey, InternalElement, Key, Element> MapFromInternalHashMap<InternalKey, InternalElement, Key, Vec<Element>> for HashMap<InternalKey, Vec<InternalElement>>
where
    Key: From<InternalKey> + std::hash::Hash + Eq,
    Element: From<InternalElement>,
{
    fn into_hash_map(self) -> HashMap<Key, Vec<Element>> {
        self.into_iter().map(|(k, v)| (Key::from(k), v.into_vec())).collect()
    }
}
...
impl<Key, Element, InternalKey, InternalElement> MapIntoInternalHashMap<Key, Element, InternalKey, InternalElement> for HashMap<Key, Element>
where
...
MapIntoInternalHashMap<Key1, Element, InternalKey1, HashMap<InternalKey2, InternalElement>> 
...
InternalKey2: std::hash::Hash + Eq,
    ...
    Element: Into<InternalElement>,
{
    fn into_internal_hash_map(self) -> HashMap<InternalKey1, HashMap<InternalKey2, InternalElement>> {
        self.into_iter().map(|(k, v)| (k.into(), v.into_internal_hash_map())).collect()
    }
}

impl<Key, Element, InternalKey, InternalElement> MapIntoInternalHashMap<Key, Element, InternalKey, Vec<InternalElement>> for HashMap<Key, Vec<Element>>
where
    InternalKey: std::hash::Hash + Eq,
    Key: Into<InternalKey>,
    Element: Into<InternalElement>,
{
    fn into_internal_hash_map(self) -> HashMap<InternalKey, Vec<InternalElement>> {
        self.into_iter().map(|(k, v)| (k.into(), v.into_internal_vec())).collect()
    }
}
