...
pub fn handle_struct(
    ...
    let test_mod_name = Ident::new(
        &format!("{}_conversion_tests", name.to_string().to_lowercase()),
        name.span(),
    );
    let common = quote! {
        impl #name {
    ...
    match data.fields {
        ...
        Fields::Named(ref fields_named) => {
            let field_from_internal_conversions: Vec<_> =
                generate_struct_field_conversions(fields_named);
            let field_into_internal_conversions: Vec<_> =
                generate_struct_internal_field_conversions(fields_named);
        ...
        Fields::Unnamed(ref field_unnamed) => {
            let field_from_internal_conversions: Vec<_> =
                generate_struct_unnamed_field_conversions(field_unnamed);
            let field_into_internal_conversions: Vec<_> =
                generate_struct_unnamed_field_internal_conversions(
                    field_unnamed,
                );
            quote! {
                impl From<#internal_name> for #name {
...
fn generate_struct_field_conversions(
...
fn generate_struct_internal_field_conversions(
    fields: &syn::FieldsNamed,
) -> Vec<proc_macro2::TokenStream> {
    fields
        .named
...
fn generate_struct_unnamed_field_conversions(
...
fn generate_struct_unnamed_field_internal_conversions(
    fields: &syn::FieldsUnnamed,
) -> Vec<proc_macro2::TokenStream> {
    fields
        .unnamed
