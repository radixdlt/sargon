...
mod tests {
	...
	fn placeholder_stokenet() {
		let persona = Persona::placeholder_stokenet_hermione();
		assert_eq!(persona.display_name.value, "Hermione".to_string());
	...
	#[test]
	fn placeholder_stokenet_other() {
		let persona = Persona::placeholder_stokenet_leia_skywalker();
		assert_eq!(persona.display_name.value, "Leia".to_string());
		assert_eq!(persona.network_id, NetworkID::Stokenet);
