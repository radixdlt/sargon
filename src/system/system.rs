        ...
        if let Some(loaded) = app_secure_storage.load_active_profile().await? {
            ...
            let device_info = match app_secure_storage.load_device_info().await? {
                ...
                None => {
                    ...
                    let new_device_info = host_info_client.create_device_info().await;
                    app_secure_storage.save_device_info(&new_device_info).await?;
                    new_device_info
                }
            };
            let entropy_client = EntropyClient::new(drivers.entropy_provider.clone());
            ...
            app_secure_storage.save_private_hd_factor_source(&private_bdfs).await?;

            let profile = Profile::new(private_bdfs.factor_source, device_info);

            app_secure_storage.save_profile_and_active_profile_id(&profile).await?;
