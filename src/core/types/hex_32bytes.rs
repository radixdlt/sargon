...
macro_rules! decl_bag_of_n_bytes {
    ($struct_name:ident, $byte_count:expr) => {
        ...
        impl $struct_name {
            ...
            /// Tries to decode the string `s` into a `$struct_name`. Will fail
            ...
            /// Instantiates a new `$struct_name` from the $byte_count bytes, by cloning them.
...
decl_bag_of_n_bytes!(Hex32Bytes, 32);
decl_bag_of_n_bytes!(Hex64Bytes, 64);
decl_bag_of_n_bytes!(Hex33Bytes, 33);
decl_bag_of_n_bytes!(Hex65Bytes, 65);
...
impl From<Hash> for Hex32Bytes {
    /// Instantiates a new `$struct_name` from the `Hash` ($byte_count bytes).
