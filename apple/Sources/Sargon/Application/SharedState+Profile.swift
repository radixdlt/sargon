...
//  
//
//  Created by Alexander Cyon on 2024-05-14.
//
...
public struct SargonKey: Sendable, PersistenceReaderKey {
	
	public func load(initialValue: Profile?) -> Profile? {
		sargonOS.profile()
	}
	
	public let id: UUID
	private let sargonOS: SargonOS
	private unowned let eventBus: EventBus
	public init(eventBus: EventBus, sargonOS: SargonOS) {
		self.id = UUID.init()
		self.eventBus = eventBus
		self.sargonOS = sargonOS
	}
	
	public typealias Value = Profile
	...
	public func subscribe(
	  ...
	  didSet: @Sendable @escaping (_ newValue: Value?) -> Void
		...
		let task = Task {
			...
			for await _ in await self.eventBus.notifications().compactMap(\.event.profileChanged) {
				guard !Task.isCancelled else { return }
				didSet(self.sargonOS.profile())
			}
		}
		return .init {
			task.cancel()
